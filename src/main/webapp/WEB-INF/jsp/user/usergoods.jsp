...
<head lang="en">
    <meta charset="UTF-8">
    <title>我的二手商品</title>
    <link href="${pageContext.request.contextPath }/statics/css/usergoods.css" rel="stylesheet"/>
    <script type="text/javascript" src="${pageContext.request.contextPath }/statics/js/jquery-1.12.4.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath }/statics/js/usergoods.js"></script>
</head>
<body>
<div class="header">
...
<div class="middle">
    ...
    <div class="middle1">
        <form action="usergoodschange.do?userId=${user.id}" method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="${goods.id}"/>
            <p><span class="span1">名称</span><input class="inp" type="text" name="title" value="${goods.title}" required onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;"/></p>
            <p><span class="span1">描述</span></p>
            <textarea class="text" name="content" required>${goods.content}</textarea>
            <div class="middle2">
                <c:if test="${not empty goods.pictureName}">
                <img class="imgs" src="${pageContext.request.contextPath}/statics/goods/${goods.pictureName}"/>
                </c:if>
                <c:if test="${empty goods.pictureName}">
                    <img class="imgs" src="${pageContext.request.contextPath}/statics/sysimgs/dianzi.png"/>
                </c:if>
            </div>
            <p><span class="span1">数量</span><input id="count" name="count" value="${goods.count}" class="inpp" type="text"/><span class="span1">价格</span><input id="price" name="price" value="${goods.price}" class="inpp" type="text"/></p>
