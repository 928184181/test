...
<head lang="en">
    <meta charset="UTF-8">
    <title>我的帖子</title>
    <link href="${pageContext.request.contextPath}/statics/css/mynews.css" rel="stylesheet"/>
    <script type="text/javascript" src="${pageContext.request.contextPath}/statics/js/jquery-1.12.4.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath}/statics/js/mynews.js"></script>
</head>
<body>
<div class="header">
...
<div class="middle">
    ...
    <c:if test="${not empty pageSupportMyNews.list}">
    <div class="middle1">
        <ul class="uul">
            <li><span class="cona0">名称</span><span class="spantime0">状态</span><span class="spantime0">发帖时间</span><span class="spancount0">操作</span></li>
            <c:forEach var="News" items="${pageSupportMyNews.list}">
                <li><a class="cona" href="${pageContext.request.contextPath}/tomynewslook.do?id=${News.id}">${News.title}</a><span class="spantime">${News.statusName}</span><span class="spantime">${News.date}</span><a class="spanlook" href="${pageContext.request.contextPath}/tomynewslook.do?id=${News.id}">查看</a><a class="spandel" href="" onclick="del(${News.id})">删除</a></li>
            </c:forEach>
        </ul>
    </div>
    <div class="page">
        <span class="pagecount">共有<span id="count">${pageSupportMyNews.totalPage}</span>页</span><a href="javascript:void(0);" id="shouye" class="apage">首页</a><a href="javascript:void(0);" id="sy" class="apage">上一页</a><span class="current">当前第</span><span id="currentPage">${pageSupportMyNews.currentPage}</span><span class="current">页</span><a href="javascript:void(0);" id="xy" class="apage1">下一页</a><a href="javascript:void(0);" id="moye" class="apage">末页</a>
    </div>
    </c:if>
    <c:if test="${empty pageSupportMyNews.list}">
    <div class="pageno">
        <p class="pno">您还没有发表帖子！</p>
    </div>
    </c:if>
</div>
<div class="footer">
