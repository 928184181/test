...
<head lang="en">
    <meta charset="UTF-8">
    <title>最新资讯</title>
    <link rel="stylesheet" href="${pageContext.request.contextPath}/statics/css/message.css"/>
...
<div class="middle">
    <div class="middle0">
        <input class="fb" type="button" id="hot" value="热度最高"/><input class="fb" type="button" id="new" value="最新发布"/>
    </div>
    <div class="middle1">
        ...
        <select id="sel">
            ...
            <option value="2">葫芦岛校区</option>
        </select>
        <p class="pcount">共<span id="totalCount">${pageSupportNews.totalCount}</span>个资讯</p>
    ...
    <div class="middle2">
        <ul class="uul">
            <li><span class="cona0">名称</span><span class="spanftr0">发帖人</span><span class="spantime0">发帖时间</span><span class="spancount0">点击量</span></li>
            <c:forEach var="News" items="${pageSupportNews.list}">
                <li><a class="cona" href="tonewslook.do?id=${News.id}">${News.title}</a><span class="spanftr">${News.userName}</span><span class="spantime">${News.date}</span><span class="spancount">${News.clicks}</span></li>
            </c:forEach>
        </ul>
    </div>
    <div class="page">
        <span class="pagecount">共有<span id="count">${pageSupportNews.totalPage}</span>页</span><a href="javascript:void(0);" id="shouye" class="apage">首页</a><a href="javascript:void(0);" id="sy" class="apage">上一页</a><span class="current">当前第</span><span id="currentPage">${pageSupportNews.currentPage}</span><span class="current">页</span><a href="javascript:void(0);" id="xy" class="apage1">下一页</a><a href="javascript:void(0);" id="moye" class="apage">末页</a>
    </div>
</div>
<div class="pageno">
