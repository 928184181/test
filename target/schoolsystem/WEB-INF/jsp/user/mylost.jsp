...
<head lang="en">
    <meta charset="UTF-8">
    <title>我的遗失物品发布</title>
    <link href="${pageContext.request.contextPath}/statics/css/mylost.css" rel="stylesheet"/>
    <script type="text/javascript" src="${pageContext.request.contextPath}/statics/js/jquery-1.12.4.js"></script>
    <script type="text/javascript" src="${pageContext.request.contextPath}/statics/js/mylost.js"></script>
</head>
<body>
<div class="header">
...
<div class="middle">
    ...
    <c:if test="${not empty pageSupportMyLost.list}">
    <div class="middle1">
        <ul class="uul">
            <li><span class="cona0">名称</span><span class="spantime0">状态</span><span class="spantime0">发布时间</span><span class="spancount0">操作</span></li>
            <c:forEach items="${pageSupportMyLost.list}" var="lost">
            <li><a class="cona" href="${pageContext.request.contextPath}/tomylostlook.do?id=${lost.id}">${lost.title}</a><span class="spantime">${lost.claimName}</span><span class="spantime">${lost.date}</span><a class="spanlook" href="${pageContext.request.contextPath}/tomylostlook.do?id=${lost.id}">查看</a><a class="spandel" href="" onclick="del(${News.id})">删除</a></li>
            </c:forEach>
        </ul>
    </div>
    <div class="page">
        <span class="pagecount">共有<span id="count">${pageSupportMyLost.totalPage}</span>页</span><a href="javascript:void(0);" id="shouye" class="apage">首页</a><a href="javascript:void(0);" id="sy" class="apage">上一页</a><span class="current">当前第</span><span id="currentPage">${pageSupportMyLost.currentPage}</span><span class="current">页</span><a href="javascript:void(0);" id="xy" class="apage1">下一页</a><a href="javascript:void(0);" id="moye" class="apage">末页</a>
    </div>
    </c:if>
    <c:if test="${empty pageSupportMyLost.list}">
    <div class="pageno">
        <p class="pno">您还没有发布遗失物品！</p>
    </div>
    </c:if>
</div>
<div class="footer">
